...
int main(int argc, char *argv[]) {
    ...
    numBytesWide =avpicture_get_size(PIX_FMT_RGB24, WIDTH*LENGTH, HEIGHT);
    bufferWide = (uint8_t *)av_malloc(numBytesWide*sizeof(uint8_t));

    numBytesBar = avpicture_get_size(PIX_FMT_RGB24, LENGTH, HEIGHT);
    ...
    sws_ctx2 = sws_getContext(WIDTH*LENGTH, HEIGHT, PIX_FMT_RGB24,
                              LENGTH, HEIGHT, PIX_FMT_RGB24, SWS_BILINEAR, NULL, NULL, NULL);

    // Assign appropriate parts of buffer to image planes in pFrameRGB
    // Note that pFrameRGB is an AVFrame, but AVFrame is a superset
    // of AVPicture
    avpicture_fill((AVPicture *)pFrameRGB, buffer, PIX_FMT_RGB24, WIDTH, HEIGHT);
    avpicture_fill((AVPicture *)pFrameWide, bufferWide, PIX_FMT_RGB24, WIDTH*LENGTH, HEIGHT);
    avpicture_fill((AVPicture *)pFrameBar, bufferBar, PIX_FMT_RGB24, LENGTH, HEIGHT);
    ...
    sws_scale(sws_ctx2, (uint8_t const * const *)pFrameWide->data, pFrameWide->linesize, 0, HEIGHT,
              pFrameBar->data, pFrameBar->linesize);

    SaveFrame(pFrameBar, LENGTH, HEIGHT);
